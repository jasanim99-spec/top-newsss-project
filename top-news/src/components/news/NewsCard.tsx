...
export function NewsCard({
  ...
  if (variant === 'sidebar') {
    return (
      <motion.div
        ...
        className={`bg-white dark:bg-slate-900 rounded-2xl p-3 border border-slate-200/80 dark:border-slate-800 shadow-xs hover:shadow-md transition-all ${className}`}
      >
        <Link to={articleUrl} className="block group">
          <div className="flex space-x-3 items-center">
            {showImage && article.imageUrl && (
              <div className="flex-shrink-0">
                <img
                  src={article.imageUrl}
                  alt={article.title}
                  className="w-16 h-16 object-cover rounded-xl border border-slate-100 dark:border-slate-800 group-hover:scale-105 transition-transform"
                  loading="lazy"
                  onError={(e) => {
                    e.currentTarget.onerror = null;
                    e.currentTarget.src = '/placeholder.svg';
            ...
            <div className="flex-1 min-w-0 space-y-1">
              ...
              <h3 className="text-xs font-black text-slate-900 dark:text-slate-100 leading-snug group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-2">
                {article.title}
              </h3>

              <div className="flex items-center text-[10px] text-slate-400 dark:text-slate-500 font-semibold space-x-2">
                <span>{timeAgo}</span>
                <span>•</span>
                <span className="flex items-center gap-1 text-slate-600 dark:text-slate-300 font-bold">
                  <Eye className="w-3 h-3 text-cyan-600 dark:text-cyan-400" /> {(article.views || 0).toLocaleString()}
                </span>
              </div>
            </div>
          </div>
  ...
  return (
    <motion.div
      ...
      className={`bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col justify-between h-full ${className}`}
    >
      <Link to={articleUrl} className="flex flex-col justify-between h-full flex-1 group">
        {showImage && article.imageUrl && (
          <div className="aspect-[16/9] relative overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 mb-4">
            <img
              src={article.imageUrl}
              alt={article.title}
        ...
        <div className="space-y-2 flex-1 flex flex-col justify-between">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 flex items-center gap-1">
              <Clock className="w-3 h-3 text-slate-400 dark:text-slate-500" /> {timeAgo}
            </span>
          </div>

          <h3 className="text-base font-black text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-2 leading-snug">
            {article.title}
          </h3>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-400 dark:text-slate-500 font-semibold">
            <div className="flex items-center space-x-1 text-slate-700 dark:text-slate-300 font-extrabold">
            ...
            {Array.isArray(article.tags) && article.tags.length > 0 && (
              <div className="flex items-center space-x-1">
                {article.tags.slice(0, 2).map((tag) => (
                  <span
                    key={tag}
                    className="truncate max-w-[80px] px-2 py-0.5 text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-md"
                  >
                    #{tag}
                  </span>
                ))}
